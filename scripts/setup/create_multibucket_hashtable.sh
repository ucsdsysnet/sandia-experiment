...
sudo tc filter add dev ens2 egress prio 99 handle 2: protocol ip u32 divisor 64
...
# sudo tc filter add dev ens4 egress protocol ip prio 5 \
#         u32 ht 800:: \
...
for i in {1..32}; do
    # x=$( printf "%x" $i ) ; 
    sudo tc filter add dev ens2 egress prio 100 u32 \
                      ht 2:$[i-1]: hashkey mask 0x000000ff at 16 \
                      match ip dst 10.100.20.$(echo "100 + $i" | bc)/24 \
                      action skbedit queue_mapping $i
done
